...
<?php if ( have_posts() ): ?>
    <?php while ( have_posts() ): the_post(); ?>
            ...
            <div class="article-area md:grid md:grid-cols-12 md:gap-5">
                <div class="article-area--sidebar md:col-span-3">
                    <ul class="font-light text-xl flex justify-center md:flex-col md:justify-start space-x-4 md:space-x-0">
                        ...
                        <li>
                            <?php echo __( 'Author', 'aydev' ); ?>:
                            <span class="italic font-bold"><?php echo get_the_author() ?></span>
                        </li>
                        <li class="hidden md:block">
                            <span class="text-sm text-gray-600"><?php echo get_the_author_meta( 'description' ) ?: 'You still not have a bio?, go to your profile setting and add one. And remember not to show your user name (it\'s set by default)' ?></span>
                        </li>
                    </ul>
                </div>
        ...
        <ul class="flex flex-col space-y-4 md:space-y-0 md:flex-row md:justify-between">
            ...
            <?php if ( $prev ): ?>
            <li><a
                title="<?php echo get_the_title($prev) ?>"
                class="flex items-center space-x-2 border-container px-4 py-2"
                href="<?php echo get_the_permalink($prev) ?>">
            ...
            <?php if ( $next ): ?>
            <li><a
                title="<?php echo get_the_title( $next ) ?>"
                class="flex items-center space-x-2 border-container px-4 py-2"
                href="<?php echo get_the_permalink( $next ) ?>">
                <span>Next</span>
                <span class="text-xl">➡️</span>
            </a></li>
            <?php else: ?>
